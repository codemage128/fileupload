...
app.get('/', async function (req, res) {
   let result = await db.collection('data').get();
   let returnData = []
   result.forEach(doc => {
      returnData.push(doc);
   });
...
app.get('/delete', async function (req, res) {
   let id = req.query.id;
   let result = await db.collection('data').doc(id).delete();
   res.redirect('/');
})

app.get('/download', async function (req, res) {
   let id = req.query.id;
   let result = await db.collection('data').doc(id).get();
...
app.post('/upload', async (req, res) => {
   ...
   const result = await db.collection('data').doc(uuidv4()).set(data);
   return res.redirect('/');
})
...
const bucketName = "image-gallery-hh.appspot.com";
